...
export default function LeadsExplorerPage() {
  const [activeTab, setActiveTab] = useState<'raw' | 'verified'>('verified');
  ...
  const toggleSelectAll = () => {
    if (activeTab === 'verified') {
      if (selectedVerifiedEmails.size === filteredVerifiedLeads.length && filteredVerifiedLeads.length > 0) {
        setSelectedVerifiedEmails(new Set());
      } else {
        const allEmails = new Set(filteredVerifiedLeads.map(l => l.decision_maker_email));
        setSelectedVerifiedEmails(allEmails);
      }
    } else {
      if (selectedRawDomains.size === filteredRawLeads.length && filteredRawLeads.length > 0) {
        setSelectedRawDomains(new Set());
      } else {
        const allDomains = new Set(filteredRawLeads.map(l => l.domain));
        setSelectedRawDomains(allDomains);
      }
    }
  };
  ...
  const getServiceBadge = (service?: string, score?: number) => {
    ...
    let icon = <Cpu className="h-3 w-3 text-indigo-400" />;
    ...
    return (
      <div className="flex flex-col gap-1">
        <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-md border ${badgeStyle}`}>
          {icon}
          {s.split('&')[0].trim()}
        </span>
        {score ? (
          <span className="text-[10px] text-slate-400 font-mono flex items-center gap-1">
            Need Score: <span className="text-emerald-400 font-bold">{score}/100</span>
          </span>
        ) : null}
      </div>
    );
  ...
  const renderPainPoints = (text?: string) => {
    if (!text) return <span className="text-slate-500 italic">—</span>;
    if (text.startsWith('[') && text.includes(']')) {
      ...
      return (
        <div className="space-y-1">
          <span className="inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold bg-cyan-500/10 text-cyan-300 border border-cyan-500/20">
            {niche}
          </span>
          <p className="text-slate-300 text-[11px] line-clamp-2 leading-relaxed" title={text}>
            {rest}
          </p>
        </div>
      );
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'READY':
        return <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30"><CheckCircle2 className="h-3 w-3" /> READY</span>;
      case 'SENT':
        return <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-primary-500/10 text-primary-400 border border-primary-500/30"><Send className="h-3 w-3" /> SENT</span>;
      case 'DONE':
        return <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/30"><CheckCircle2 className="h-3 w-3" /> DONE</span>;
      case 'PENDING':
        return <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30"><Clock className="h-3 w-3" /> PENDING</span>;
      case 'REJECTED':
      case 'BOUNCED':
        return <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-rose-500/10 text-rose-400 border border-rose-500/30"><XCircle className="h-3 w-3" /> {status}</span>;
      default:
        return <span className="text-[11px] font-semibold px-2 py-0.5 rounded bg-slate-800 text-slate-300">{status}</span>;
    }
  };
  ...
  return (
    <div className="flex-1 p-6 space-y-6 max-w-7xl w-full mx-auto">
      ...
      {deleteMessage && (
        <div className="flex items-center justify-between p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-semibold animate-fade-in shadow-lg">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-extrabold text-white tracking-tight flex items-center gap-2">
            <Users className="h-5 w-5 text-primary-400" /> Leads Database Explorer
          </h1>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-surface-100/80 hover:bg-surface-50 border border-white/5 text-xs text-slate-300 transition-all"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-surface-100 hover:bg-surface-50 border border-white/10 text-xs font-semibold text-white transition-all shadow-sm"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-xs font-bold text-white transition-all shadow-lg shadow-emerald-600/20"
          >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 glass-card p-4 rounded-2xl border border-white/5">
        {/* Switch Sheet 1 vs Sheet 2 */}
        <div className="flex p-1 bg-surface-200/80 rounded-xl border border-white/5 shrink-0">
          <button
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto flex-wrap">
          ...
          {selectedCount > 0 && (
            <button
              ...
              disabled={isDeleting}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold transition-all shadow-lg shadow-rose-600/30 animate-pulse-subtle"
            >
          ...
          <div className="relative flex-1 sm:w-52">
            ...
            <input
              ...
              className="w-full bg-surface-200 border border-white/10 rounded-xl pl-9 pr-3.5 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-primary-500"
            />
          </div>
          ...
          <select
            ...
            className="bg-surface-200 border border-white/10 rounded-xl px-2.5 py-2 text-xs text-slate-200 focus:outline-none focus:border-primary-500"
          >
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <select
              ...
              className="bg-surface-200 border border-white/10 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-primary-500"
            >
      ...
      <div className="glass-card rounded-2xl border border-white/5 overflow-hidden shadow-xl">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-3 text-slate-500">
            ...
            <p className="text-xs">Querying Google Sheets Primary Database...</p>
          </div>
        ) : activeTab === 'verified' ? (
          /* Table for Sheet 2: Verified Outreach Queue */
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-surface-200/90 text-[11px] uppercase tracking-wider text-slate-400 border-b border-white/5 font-semibold">
                <tr>
                ...
                {filteredVerifiedLeads.length === 0 ? (
                  ...
                  filteredVerifiedLeads.map((row) => {
                    ...
                    return (
                      <tr 
                        key={row.row_index} 
                        className={`transition-colors ${isSelected ? 'bg-primary-500/10' : 'hover:bg-white/[0.02]'}`}
                      >
                        ...
                        <td className="px-3 py-3 font-mono text-slate-500">{row.row_index}</td>
                        ...
                        <td className="px-4 py-3 font-mono text-slate-400 text-[11px]">{row.sent_timestamp || '—'}</td>
                        <td className="px-4 py-3 text-right">
                          <div className="inline-flex items-center gap-2 justify-end">
                            {row.outreach_status === 'READY' && (
                              <button
                                onClick={() => openApprovalModal(row)}
                                className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-primary-600/20 hover:bg-primary-600/30 text-primary-300 border border-primary-500/30 text-xs font-semibold transition-all shadow-sm"
                              >
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          ...
                          <span className="text-[11px] text-slate-400 block">{row.company_name}</span>
                          {row.phone_number && (
                        ...
                        <td className="px-4 py-3 font-mono text-cyan-300 max-w-xs">
                          {row.raw_email ? (
